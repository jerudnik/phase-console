...
# ---------------------------------------------------------------------------
# Normalized fixture data from a real Kanidm login flow
# ---------------------------------------------------------------------------

KANIDM_BASE_URL = "https://auth.example.com"

OIDC_DISCOVERY_RESPONSE = {
    "token_endpoint": f"{KANIDM_BASE_URL}/api/oidc/token",
    "authorization_endpoint": f"{KANIDM_BASE_URL}/api/oidc/authorization",
    "userinfo_endpoint": f"{KANIDM_BASE_URL}/api/oidc/userinfo",
    "jwks_uri": f"{KANIDM_BASE_URL}/jwks.json",
    "issuer": KANIDM_BASE_URL,
}
...
def _make_mock_token(access_token="fake-access-token", id_token=None):
    ...
    if id_token is None:
        # Simulate the real Kanidm path: token.id_token is None
        del token.id_token
    else:
...
# ---------------------------------------------------------------------------
# Tests for KanidmOpenIDConnectAdapter static configuration
# ---------------------------------------------------------------------------


class TestKanidmAdapterConfiguration(unittest.TestCase):
    """Verify that the Kanidm adapter has correct static configuration."""

    @patch.dict(
        "os.environ",
        {"KANIDM_URL": KANIDM_BASE_URL},
    )
    @patch(
    ...
    def test_provider_id(self, mock_get):
        ...
        import importlib
        import api.authentication.providers.kanidm.views as kanidm_views

        # Assert
        self.assertEqual(kanidm_views.KanidmOpenIDConnectAdapter.provider_id, "kanidm")
    ...
    def test_default_config_urls(self, mock_get):
        ...
        # Since KANIDM_URL was empty at import time, the URLs will use the
        # empty string base. We check the keys exist and the structure is right.
        default_config = kanidm_views.KanidmOpenIDConnectAdapter.default_config
...
class TestOIDCDiscovery(unittest.TestCase):
    ...
    def test_successful_oidc_discovery(self, mock_get):
        ...
        from api.authentication.providers.kanidm.views import (
            KanidmOpenIDConnectAdapter,
        )

        adapter = KanidmOpenIDConnectAdapter.__new__(KanidmOpenIDConnectAdapter)
        adapter.oidc_config_url = f"{KANIDM_BASE_URL}/.well-known/openid-configuration"
    ...
    def test_oidc_discovery_failure_falls_back_to_defaults(self, mock_get):
        ...
        adapter.oidc_config_url = f"{KANIDM_BASE_URL}/.well-known/openid-configuration"
        adapter.default_config = {
            "access_token_url": f"{KANIDM_BASE_URL}/api/oidc/token",
            "authorize_url": f"{KANIDM_BASE_URL}/api/oidc/authorization",
            "profile_url": f"{KANIDM_BASE_URL}/api/oidc/userinfo",
            "jwks_url": f"{KANIDM_BASE_URL}/jwks.json",
            "issuer": KANIDM_BASE_URL,
        }
        ...
        self.assertEqual(adapter.access_token_url, f"{KANIDM_BASE_URL}/api/oidc/token")
        self.assertEqual(adapter.authorize_url, f"{KANIDM_BASE_URL}/api/oidc/authorization")
        self.assertEqual(adapter.profile_url, f"{KANIDM_BASE_URL}/api/oidc/userinfo")
        self.assertEqual(adapter.jwks_url, f"{KANIDM_BASE_URL}/jwks.json")
        self.assertEqual(adapter.issuer, KANIDM_BASE_URL)
...
class TestCompleteLogin(unittest.TestCase):
    ...
    def _make_adapter(self):
        ...
        adapter = KanidmOpenIDConnectAdapter.__new__(KanidmOpenIDConnectAdapter)
        adapter.profile_url = f"{KANIDM_BASE_URL}/api/oidc/userinfo"
        adapter.jwks_url = f"{KANIDM_BASE_URL}/jwks.json"
        adapter.issuer = KANIDM_BASE_URL
        adapter.access_token_url = f"{KANIDM_BASE_URL}/api/oidc/token"
        adapter.authorize_url = f"{KANIDM_BASE_URL}/api/oidc/authorization"
        return adapter
    ...
    def test_complete_login_userinfo_fallback_when_id_token_is_none(self, mock_get):
        """
        The real Kanidm path: token.id_token is None, so the adapter
        ...
        mock_get.assert_called_once_with(
            f"{KANIDM_BASE_URL}/api/oidc/userinfo",
    ...
    def test_complete_login_with_id_token_from_kwargs_response(self, mock_jwk_client_cls, mock_jwt_decode):
        ...
        mock_jwk_client_cls.assert_called_once_with(f"{KANIDM_BASE_URL}/jwks.json", headers={"User-Agent": "phase-console"})
    ...
    def test_complete_login_with_id_token_on_token_object(self, mock_jwk_client_cls, mock_jwt_decode):
        """
        When token.id_token is set directly (not the typical Kanidm path,
...
class TestPreSocialLogin(unittest.TestCase):
    ...
    def _make_sociallogin(self, email="testuser@example.com", uid="79e01414-21b4-49b1-9b61-f9774b980350", provider="kanidm"):
